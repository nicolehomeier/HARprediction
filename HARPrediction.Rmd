...
Overview: The first set of steps is reducing the set of variables for prediction to only those that are useful. To do this I removed variables that are missing in the test data, variables where most are missing in the training data, and variables that likely do not contribute much predictive power. The next steps are training and evaluating models. I split the training set into a 60%/40% split for training/testing, then fit 4 models: a tree model, a linear discriminant analysis model, a nearest neighbors (nn) model, and a random forest (rf). The tree, nn, and rf models all had good accuracy in this evaluation, with the rf having the least error. I chose the rf to proceed with, fit a final model using all the training data, and predicted on the 20 row test set. I expected the error rate on this small set of data to be near zero, so could be that every case was predicted correctly. From the larger 60/40 split the expected error rate is less than 1%. 
```{r load}
library(caret)
...
now use all the data for the final fit and quiz prediction

```{r finalrffit}
finalfit = randomForest(classe~.,data=cnds,mtry=6,ntree=200,keep.forest=TRUE)
